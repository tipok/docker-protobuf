...
FROM swift:5.0.1 as swift_builder

ENV GRPC_SWIFT_VERSION=0.9.0

RUN apt-get install -y curl patchelf && \
        mkdir -p /swift-protobuf && \
        curl -L https://github.com/grpc/grpc-swift/archive/${GRPC_SWIFT_VERSION}.tar.gz | tar --strip-components 1 -C /swift-protobuf -xz
RUN apt-get install -y libcurl4-openssl-dev
RUN cd /swift-protobuf && \
        make plugin
RUN mkdir -p /protoc-gen-swift && \
        cp /swift-protobuf/protoc-gen-swift* /protoc-gen-swift/ 
...
RUN find /protoc-gen-swift/ -name 'lib*.so*' -exec patchelf --set-rpath /protoc-gen-swift {} \; && \
        for p in $(ls /protoc-gen-swift/protoc-gen-swift*); do \
        echo ${p}; \
        patchelf --set-interpreter /protoc-gen-swift/ld-linux-x86-64.so.2 ${p}; \
        done
